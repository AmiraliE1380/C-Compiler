...
from token import is_nonterminal,syntax_error_terminal,token_to_terminal,stringify_token
...
class Parser:
    ...
    def increment_state(self):
        ...
        while current_state[2] == len(self.grammar[current_state[0]][current_state[1]]):
    ...
    def match_rule(self,nonterminal):
        if nonterminal not in self.grammar:
            ...
            return
        epsilon_index = -1
        for transition_index in range(len(self.grammar[nonterminal])):
            first_transition = self.grammar[nonterminal][transition_index][0] 
            if is_nonterminal(first_transition):
                if self.terminal_match_nonterminal(self.lookahead_terminal, first_transition):
                    return 1,transition_index, 'Nonterminal Matching'
                if self.terminal_match_nonterminal("epsilon", first_transition) and self.lookahead_terminal in self._grammer.follow_set(first_transition):
                    epsilon_index = transition_index
            else:
                if first_transition == 'epsilon' and self.lookahead_terminal in self._grammer.follow_set(nonterminal):
                    epsilon_index = transition_index
                elif self.lookahead_terminal == first_transition:
    ...
    def run_dfa(self):
        while True:
            ...
            if current_state[1] == -1:
                if len(self.grammar[current_state[0]]) > 1:
                    ...
                    current_state[1] = 0
                    current_state[2] = 0
            current_transition =  self.grammar[current_state[0]][current_state[1]][current_state[2]]
            ...
            elif current_transition == self.lookahead_terminal:
                Node(stringify_token(self.lookahead_token),parent=current_state[3])
                self.increment_state()
    ...
    def panic_mode(self,result):
        lineno = self.lookahead_token[2]
        if result == -2:
            # missing a character
            missed_token = self.grammar[self.state_stack[-1][0]][self.state_stack[-1][1]][self.state_stack[-1][2]]
            ...
            if self.lookahead_terminal in self._grammer.follow_set(curr_var):
                ...
                if self.lookahead_terminal != '$':
                    self.error_strings.append('#'+str(lineno) + ' : syntax error, illegal ' + syntax_error_terminal(self.lookahead_terminal) + '\n')
                    return True
                else:
    ...
    def parser_driver(self,input_prog):
        ...
        get_next_token = self.scanner(input_prog)
        get_new_token = True
        while len(self.state_stack) > 0:
            if get_new_token:
                self.lookahead_token = get_next_token()
                self.lookahead_terminal = token_to_terminal(self.lookahead_token)
    ...
    def terminal_match_nonterminal(self,terminal,nonterminal):
        return terminal in self._grammer.first_set(nonterminal)